...
import {
  ...
  Switch, Alert, ActivityIndicator, Platform, StatusBar
} from 'react-native';
import { LinearGradient } from 'expo-linear-gradient';
import { Ionicons } from '@expo/vector-icons';
import AsyncStorage from '@react-native-async-storage/async-storage';
import NetInfo from '@react-native-community/netinfo';
import { obtenerHistoriasPendientes, obtenerTodasLasHistorias } from '../baseDatosLite/basedatoslt';
...
export default function AjustesScreen({ navigation }) {
  ...
  async function handleSync() {
    ...
    await cargarDatos();
    Alert.alert(
      resultado.success ? 'Sincronización completada' : 'Error al sincronizar',
      resultado.message,
    );
  }

 async function limpiarSincronizadas() {
    Alert.alert(
      'Limpiar datos sincronizados',
      'Se eliminarán del dispositivo las historias que ya fueron sincronizadas con el servidor.',
      [
        { text: 'Cancelar', style: 'cancel' },
        {
          ...
          style: 'destructive',
          onPress: async () => {
            const { eliminarSincronizadas } = await import('../baseDatosLite/basedatoslt');
            await eliminarSincronizadas();
            await cargarDatos();
            Alert.alert('Listo', 'Historias sincronizadas eliminadas del dispositivo.');
          },
        },
      ]
    );
  ...
  async function borrarTodosLosDatos() {
    Alert.alert(
      '⚠️ Borrar todo',
      '¿Estás seguro? Se eliminarán TODAS las historias del dispositivo incluyendo las no sincronizadas. Esta acción no se puede deshacer.',
      [
        { text: 'Cancelar', style: 'cancel' },
        {
          text: 'Sí, borrar todo',
          ...
          onPress: () => {
            Alert.alert(
              ...
              'Se perderán todas las historias locales.',
              [
                { text: 'Cancelar', style: 'cancel' },
                {
                  text: 'Borrar todo',
      ...
      {/* Datos */}
        <View style={styles.seccion}>
          ...
          <Accion
            ...
            sub="Elimina del dispositivo las historias ya subidas"
            color={COLORES.error}
            onPress={limpiarSincronizadas}
          />
          <View style={styles.separador} />
          <Accion
            icono="trash-outline"
            label="Borrar todos los datos"
            sub="Elimina TODAS las historias del dispositivo"
            color={COLORES.error}
        ...
        <View style={styles.seccion}>
          ...
          {[
            ...
            { label: 'IA en la nube',   valor: 'Groq · llama-3.1-8b' },
          ].map(({ label, valor }) => (
